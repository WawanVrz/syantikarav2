<?php include "Header.php"; ?>
          ...
          <div class="box box-primary">
            ...
            <div class="box-body">
              ...
              <p style="margin-left:10px;">
                <a href="<?php echo base_url('dashboard/admin/data/konfirmasi/checkout'); ?>"><button class="btn btn-success"><i class="glyphicon glyphicon-chevron-left"></i>&nbsp; Back To View</button></a>
                <?php if($dr->status_pembayaran == "Belum Lunas"){ ?>
                  <a href="<?php echo base_url('dashboard/admin/data/reservasi/pembayaran/'.$dr->id_transaksi); ?>"><button class="btn btn-success"><i class="glyphicon glyphicon-credit-card"></i>&nbsp; Bayar Tagihan</button></a>
                <?php }else{ ?>

                <?php } ?>
                <?php if($dr->status_order != "Check Out"){ ?>
                  <button class="btn btn-success" onClick="checkoutdata(<?= $dr->id_transaksi ?>)"><i class="glyphicon glyphicon-ok"></i>&nbsp; Check Out</button>
                  <?php }else{ ?>
  ...
  function PaymentData(id) {
      swal({
        ...
        text: "Anda Yakin Ingin Membayar Tagihan Reservasi",
        type: "warning",
      ...
      function() {
          $.get('<?php echo base_url('ReservationController/payment_reservasi/'); ?>', {id:id}, function(data) {
            if (data == 'succeed') {
                //$('#row-' + id).remove();
                swal("Lunas!", "Tagihan Anda Sudah Lunas", "success");
            }
  ...
  function checkoutdata(id) {
      ...
      function() {
          $.get('<?php echo base_url('KonfirmasiController/checkoutproses/'); ?>', {id:id}, function(data) {
            if (data == 'succeed') {
                //$('#row-' + id).remove();
                swal("Success!", "Your Has Been Check Out !", "success");
            }
            window.location.reload();
          });
      });
