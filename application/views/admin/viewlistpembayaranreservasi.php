              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                  <?php
                    ...
                    foreach ($reservasi as $r)
                    {
                      ...
                      $angka3 = $r->total_bayar2;
                      $angka_format3 = number_format($angka3,2,",",".");
                      $angka2 = $r->dp;
                  ...
                  ?>
                        <tr id="row-<?= $r->id_transaksi ?>">
                          <td><?php echo $no++ ?></td>
                          <td><?php echo $r->instansi."/".$r->nama_pemesan  ?></td>
                ...
                ?>
                    <tr id="row-<?= $r->id_transaksi ?>">
                      ...
                      <td><?php echo $format2 ?></td>
                      <td><?php echo $r->status_order ?></td>
                      <td>Rp <?php echo $angka_format3 ?></td>
                      <td><?php echo $r->status_pembayaran ?></td>
                      <td align="center">
                        <table>
                          <tr>
          ...
          <form id="MyForm2" enctype="multipart/form-data">
            <table border="0" width="100%">
              ...
              <tr class="t1">
                ...
                <td height="43" colspan="3"><input type="text" id="totalbayar" class="form-control" value='' readonly><input type="hidden" id="totalbayarint" name="totalint" class="form-control int1"></td>
                <td width="70%"></td>
              </tr>
              <tr class="t2">
                <td width="30%" height="42"><b> Total Bayar </b></td>
                <td height="43" colspan="3"><input type="text" id="totalbayar2" class="form-control" readonly><input type="hidden" id="totalbayarint2" name="totalint" class="form-control int2"></td>
                <td width="70%"></td>
              </tr>
              <tr>
              ...
              <tr class="cekafter2">
                ...
                <td height="43" colspan="3">
                  <input type="text" id="reduksiAfter" name="reduksiAfterstr" class="form-control" readonly>
                  <input type="hidden" id="reduksiAfterint" name="reduksiafter" class="form-control">
                </td>
                <td width="70%"></td>
              </tr>
              <tr class="t3">
                <td width="30%" height="41"><b> Jumlah Terbayarkan </b></td>
                <td height="43" colspan="3"><input type="text" id="terbayarakan" class="form-control" readonly></td>
                <td width="70%"></td>
              </tr>
              <tr class="t5">
                <td width="30%" height="41"><b> Jumlah Terbayarkan </b></td>
                <td height="43" colspan="3"><input type="text" id="terbayarakanafter" class="form-control" readonly></td>
                <td width="70%"></td>
              </tr>
              <tr class="t4">
                <td width="30%" height="41"><b> Baru Terbayarkan </b></td>
                <td height="43" colspan="3"><input type="text" id="barudibayar" class="form-control" readonly></td>
                <td width="70%"></td>
              </tr>
              <tr>
                <td colspan="2"> <hr> </td>
  ...
  <script>
    ...
    function prosesbayar($idtransaksi)
    {
      ...
      $.get('<?php echo base_url(). 'api/transaksi/pembayaran'; ?>', {id_transaksi: id_transaksi}, function(response)
      {
        ...
        total1 =  data.total_bayar;
        total2 =  data.total_bayar2;
        console.log(total1);
        console.log(total2);
        ...
        $('.t2').hide();
          if(reduksi != "" && tunai != "")
          {
               ...
               if(reduksicek == 0)
               {
                  ...
                  var total2 = data.total_bayar2;
                  var rptotal2 = toRp(total2);
                  var dp = data.dp;
                  var rptotaldp = toRp(dp);
                  var terbayar = data.Jumlah_Terbayarkan;
                  var rpterbayar = toRp(terbayar);
                  var tunaix = data.tunai;
                  var rptunaix = toRp(tunaix);
                 ...
                 $('#terbayarakan').val(rpterbayar);
                 $('#reduksiAfter').val(rpterreduksi);
                 $('#reduksiAfterint').val(terreduksi);
                 $('#barudibayar').val(rptunaix);

                   var totalAll = (parseFloat(total) - parseFloat(dp) - parseFloat(terreduksi) - parseFloat(tunaix));
                   var totalkembalian = (parseFloat(tunai) - parseFloat(totalAll));
                   var rp = toRp(totalAll);
                   var rp2 = toRp(totalkembalian);
                   $('#totalallint').val(totalAll);
         ...
         $('.t1').hide();
            ...
            var reduksi = reduksi.replace(/[^,\d]/g, '').toString();
            tunai = tunai.replace(/[^,\d]/g, '').toString();
            var data = JSON.parse(response);
            var reduksicek = data.reduksi;
            var pi = data.piutang;
          ...
          if(pi == 0)
          {
            if(reduksicek == 0)
            {
              ...
              $('#barudibayar').val(rptunaix);
                ...
                var totalAll = (parseFloat(total2) - parseFloat(dp) - parseFloat(reduksi) - parseFloat(terbayar));
                var totalkembalian = (parseFloat(tunai) - parseFloat(totalAll));
                var rp = toRp(totalAll);
                var rp2 = toRp(totalkembalian);
                $('#totalallint').val(totalAll);
              ...
              $('#barudibayar').val(rptunaix);
                ...
                var totalAll = (parseFloat(total2) - parseFloat(dp) - parseFloat(terreduksi) - parseFloat(terbayar));
                var totalkembalian = (parseFloat(tunai) - parseFloat(totalAll));
                var rp = toRp(totalAll);
                var rp2 = toRp(totalkembalian);
                $('#totalallint').val(totalAll);
            ...
            if(reduksicek == 0)
            {
              ...
              var terbayar = data.Jumlah_Terbayarkan;
              var rpterbayar = toRp(terbayar);
              var terbayara = data.Jumlah_TerbayarkanAfter;
              var rpterbayara = toRp(terbayara);
              var tunaix = data.tunai;
              var rptunaix = toRp(tunaix);
              ...
              $('#bayardpint').val(dp);
              $('#terbayarakan').val(rpterbayar);
              $('#terbayarakanafter').val(rpterbayara);
              $('#reduksiAfter').val(rpterreduksi);
              $('#reduksiAfterint').val(terreduksi);
              $('#barudibayar').val(rptunaix);

                var totalAll = (parseFloat(total2) - parseFloat(dp) - parseFloat(terreduksi) - parseFloat(terbayar) - parseFloat(tunaix));
                var totalkembalian = (parseFloat(tunai) - parseFloat(totalAll));
                var rp = toRp(totalAll);
                var rp2 = toRp(totalkembalian);
                $('#totalallint').val(totalAll);
